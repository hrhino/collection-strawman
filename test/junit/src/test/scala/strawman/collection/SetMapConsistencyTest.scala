...
import strawman.collection.{mutable => cm, immutable => ci}
...
// Commented because we don’t have Java converters yet
...
//  }
  ...
  def extraFilterTests(): Unit = {
    type M = strawman.collection.Map[Int, Boolean]
    val manyKVs = ci.Range.inclusive(0, 1000).map(i => i*i*i).map(x => x -> ((x*x*x) < 0))
...
//    def mohm: M = { val m = new cm.OpenHashMap[Int, Boolean]; m ++= manyKVs; m }
    ...
    def pick(m: M, density: Double) = m.keys.filter(_ => rn.nextDouble < density).to(Set)
    def test: Boolean = {
      for (i <- 0 to 100) {
  ...
  @Test
  def testSI8264(): Unit = {
    val hs = ci.Set(-2147483648, 1, -45023380, -1, 1971207058, -54312241, -234243394) - -1
    assert( hs.to(List).to(Set) == hs )
    assert( hs == hs.to(List).to(Set) )
  }
...
// Disabled because filterKeys does not support Map operations (it returns a plain View)
//  @Test
